...
LOCAL blankLine IS " ":PADRIGHT(TERMINAL:WIDTH).
...
LOCAL gameData IS LEX("inProgres",FALSE, "curentTurn","X", "showOptions",FALSE,
"X",LEX("isHuman",FALSE,"AItype","nnet","AIstate",LIST(0,0),"n0Neurons",LIST()),
"O",LEX("isHuman",FALSE,"AItype","tmap","AIstate",LIST(0,0),"n0Neurons",LIST())).
LOCAL otherPlayer IS LEX("X","O","O","X").

//data structures needed for neural network AI
CLEARSCREEN.
PRINT "AI init".
//LOCAL aiData IS READJSON("0:/Ur Genetic/Info run1 results/gen1/AIdata.json")[2].
LOCAL gen IS 61.
LOCAL aiData IS READJSON("0:/Ur Genetic/Info/gen" + gen + "/AIdata.json")[0].
//32[0] is 73(nn)/200(ta)
//38[0] is 86(nn)/200(ta)
//61[0] is 57(nn)/200(ta)
LOCAL xMap IS aiData:SUBLIST(0,48).
LOCAL oMap IS xMap.
LOCAL n1GeneralParameters IS LEXICON().//LEXICON("X",LIST(),"O",LIST()).
LOCAL n0Neurons IS LEXICON("X",LIST(),"O",LIST()).
n1GeneralParameters:ADD("X",aiData:SUBLIST(48,408 * 2)).
n1GeneralParameters:ADD("O",n1GeneralParameters["X"]).
LOCAL conversionMap IS generate_conversion_map(xMap,oMap).
// LOCAL xMap IS LIST().
// UNTIL xMap:LENGTH >= 48 { xMap:ADD(RANDOM() * 2000 - 1000).}
// LOCAL oMap IS LIST().
// UNTIL oMap:LENGTH >= 48 { oMap:ADD(RANDOM() * 2000 - 1000).}
// PRINT "bord to n0 maps done".
// LOCAL n0Neurons IS LEXICON("X",LIST(),"O",LIST()).
// LOCAL n1GeneralParameters IS LEXICON("X",LIST(),"O",LIST()).
// //n1 parameter lenght generated by n0Neurons:length * (n1Neurons + 1) = 24 * (16 + 1)
// UNTIL n1GeneralParameters["X"]:LENGTH >= 408 { n1GeneralParameters["X"]:ADD(RANDOM() * 2000 - 1000). }
// PRINT "n1x parameters done".
// UNTIL n1GeneralParameters["O"]:LENGTH >= 408 { n1GeneralParameters["O"]:ADD(RANDOM() * 2000 - 1000). }
// PRINT "n1o parameter done".
// LOCAL conversionMap IS generate_conversion_map(xMap,oMap).
...
"hare",ai_hare_move@,
"tmap",ai_threat_map@,
"nnet",ai_neural_network@,
"rand",ai_random@,
"mute",ai_mutating@,
"randAI",ai_random_ai@).

LOCAL aiTypeToNum IS LEX("tort",0,"hare",1,"tmap",2,"nnet",3,"rand",4,"mute",5,"randAI",6).
...
IF NOT quitGame {
  ...
  IF NOT quitGame {
	...
	neural_network_init(n0Neurons,conversionMap).
  }
}
LOCAL oWins IS 0.
LOCAL xWins IS 0.
UNTIL quitGame {//(gamePieceData["xEnd"] >= 7) OR (gamePieceData["oEnd"] >= 7) {
  IF (gamePieceData["xEnd"] >= 7) OR (gamePieceData["oEnd"] >= 7) {
    IF gamePieceData["xEnd"] >= 7 {
      ui_congratulation("X").
	  SET xWins TO xWins + 1.
    } ELSE {
      ui_congratulation("O").
	  SET oWins TO oWins + 1.
    }
    clear_board(gameBoardData,gamePieceData).
    ...
    draw_picses(boardPos[0],boardPos[1],gameBoardData,gamePieceData).
    set_player_move_first().
	neural_network_init(n0Neurons,conversionMap).//set up n0 data
	IF oWins > 200 {
		SET oWins TO oWins - 1.
		BREAK.
	}
	PRINT "xWins: " + xWins + "  oWins: " + oWins AT(0,2).
  ...
  IF validMove {
	...
	neural_network_update(n0Neurons,moveList[selectedMove],conversionMap).
  } ELSE {
...
FUNCTION ui_help_ai {
  LOCAL col IS 0.
  LOCAL row IS 0.
  LOCAL iMax IS aiMoveMap:LENGTH - 1.
...
FUNCTION draw_game_options {
  ...
  SET row TO row + 1.
  SET row TO row + draw_human_ai_options(col,row,"X").
  SET row TO row + draw_human_ai_options(col,row,"O").
  ...
  IF gameData["inProgres"] {
    ...
    PRINT "This [-] marks the current setttings" AT(col,row + 2).
...
FUNCTION draw_human_ai_options {
  ...
  PRINT "--[ ]Tortoise" AT(col,row + 3).
  PRINT "--[ ]Hare" AT(col,row + 4).
  PRINT "--[ ]Threat Aware" AT(col,Row + 5).
  PRINT "--[ ]Neural Network" AT(col,Row + 6).
  PRINT "--[ ]Random" AT(col,row + 7).
  PRINT "--[ ]Mutating" AT(col,row + 8).
  PRINT "--[ ]Random AI" AT(col,row + 9).
  RETURN 11.
}
...
FUNCTION draw_ai_types_list {
  ...
  PRINT "[ ]Neural Network"AT(col,row + 3).
  PRINT "[ ]Random Move"AT(col,row + 4).
  PRINT "[ ]Mutating"AT(col,row + 5).
  PRINT "[ ]Random AI"AT(col,row + 6).
  PRINT "Press <Enter> to Return to Main Help Menu."AT(col,row + 7).
}
...
FUNCTION draw_ai_types_info {
  ...
  IF page = 0 {
    PRINT "The `Tortoise AI` will move it's piece closest to the start." AT(col,row).
    PRINT "This AI is an easiest difficultly AI." AT(col,row + 1).
  } ELSE IF page = 1 {
    PRINT "The `Hare AI` will move it's piece closest to the end." AT(col,row).
    PRINT "This AI is an easy difficultly AI." AT(col,row + 1).
  } ELSE IF page = 2 {
    PRINT "The `Threat Aware` AI mostly moves in response to the threat that it's" AT(col,row).
    PRINT "  opponent's pieces represent to it and tries to minimize said threat." AT(col,row + 1).
    PRINT "This AI is a normal difficultly AI." AT(col,row + 2).
  } ELSE IF page = 3 {
    PRINT "The `Neural Network` AI uses a neural network to figure out which of" AT(col,row).
    PRINT "   it's possible moves is the best." AT(col,row + 1).
    PRINT "This AI is a hard difficultly AI." AT(col,row + 2).
  } ELSE IF page = 4 {
    PRINT "The 'Random Move' AI will move a piece at random." AT(col,row).
    PRINT "This AI is an easy difficultly AI." AT(col,row + 1).
  } ELSE IF page = 5 {
    PRINT "The 'Mutating' AI will play as Hare, Tortoise, Threat Aware, " AT(col,row).
...
FUNCTION ai_threat_map {
  ...
  IF moveList:LENGTH > 1 {
    FOR move IN moveList {
      ...
      LOCAL currentThreat IS threat_at_tile(pType,moveStart).
      LOCAL futureThreat IS threat_at_tile(pType,moveEnd).
...
//nn notes
//build the input node set from the board by choosing between 3 numbers based on what is in a tile and the offboard piece info
//  should be around 20 nodes for the board and 4 for piece info, total of 24
//   node conversion will require a list of 20 lists
//    each list will be between 2 and 3 items depending on if they are for player reserved areas or the main track (2 for player reserved and 3 for main), will need 6*2(player start/end) + 6*2(aponent start/end) + 8*3(main track) values
//  output will be a list of 16 items for each space that can be moved into by the given player
//   index 0-4 will be start area, 5-12 will be main area, 13-15 will be end area
//    index 0 is off board start and index 15 is off board end
//a given move will determined by which ever move has the greatest difference in value between the output nodes
//  if to moves have equal value then a random roll will pick between
FUNCTION ai_neural_network {
  PARAMETER pType,pData,moveList.
  LOCAL n1Parameters IS n1GeneralParameters[pType].
  
  IF moveList:LENGTH > 1 {
    LOCAL p0Neurons IS n0Neurons[pType].
	
    LOCAL moveScores IS LIST().
    LOCAL greatistScore IS -2.
	//PRINT " ".
    FOR move IN moveList {
	  LOCAL current IS final_node(p0Neurons,n1Parameters,pos_to_num(move[0]) + 5).
	  LOCAL future IS final_node(p0Neurons,n1Parameters,pos_to_num(move[1]) + 5 + 16).
	  LOCAL score IS current - future.
    ...
    LOCAL shouldMove IS LIST().
    FROM { LOCAL i IS moveScores:LENGTH - 1. } UNTIL i < 0 STEP { SET i TO i - 1. } DO {
      IF moveScores[i] = greatistScore {
        shouldMove:ADD(i).
      }
    }
...
//xMap and oMap need to be lists 6*2 + 6*2 + 8*3 = 48 items in length
FUNCTION generate_conversion_map {
  PARAMETER xMap,oMap.
  LOCAL conversionMap IS LEXICON("X",LEXICON(),"O",LEXICON()).

  LOCAL commonTiles IS LIST("Main 0","Main 1","Main 2","Main 3","Main 4","Main 5","Main 6","Main 7").
  LOCAL xTiles IS LIST("X Start -4","X Start -3","X Start -2","X Start -1","X End 8","X End 9").
  LOCAL oTiles IS LIST("O Start -4","O Start -3","O Start -2","O Start -1","O End 8","O End 9").
  FOR pType IN LIST("X","O") {
    LOCAL pcMap IS conversionMap[pType].
    LOCAL pMap IS CHOOSE xMap IF pType = "X" ELSE oMap.
    LOCAL notpType IS otherPlayer[pType].
    LOCAL pTiles IS CHOOSE xTiles IF pType = "X" ELSE oTiles.
    LOCAL notpTiles IS CHOOSE oTiles IF pType = "X" ELSE xTiles.

    LOCAL i IS 4.
    LOCAL j IS 0.

    FOR pos IN pTiles {
      pcMap:ADD(pos,LIST(i,LEXICON(" ",segmoid(pMap[j]),pType,segmoid(pMap[j + 1])))).
      SET i TO i + 1.
      SET j TO j + 2.
    }
	
	FOR pos IN notpTiles {
      pcMap:ADD(pos,LIST(i,LEXICON(" ",segmoid(pMap[j]),notpType,segmoid(pMap[j + 1])))).
      SET i TO i + 1.
      SET j TO j + 2.
	}

    FOR pos IN commonTiles {
      pcMap:ADD(pos,LIST(i,LEXICON(" ",segmoid(pMap[j]),pType,segmoid(pMap[j + 1]),notpType,segmoid(pMap[j + 2])))).
      SET i TO i + 1.
      SET j TO j + 3.
    }
  }
...
FUNCTION neural_network_init {
  PARAMETER n0Neurons,conversionMap.

  FOR pType IN LIST("X","O") {//add the first 4 neurons for x and o AIs based on current off board pieces
    LOCAL notpType IS otherPlayer[pType].

    LOCAL pconMap IS conversionMap[pType].
    LOCAL pNeurons IS n0Neurons[pType].
    UNTIL pNeurons:LENGTH >= 24 { pNeurons:ADD(0). }

    SET pNeurons[0] TO gamePieceData[pType + "Start"] / 4.5 - 1.
    SET pNeurons[1] TO gamePieceData[pType + "End"] / 3 - 1.
    SET pNeurons[2] TO gamePieceData[notpType + "Start"] / 4.5 - 1.
    SET pNeurons[3] TO gamePieceData[notpType + "End"] / 3 - 1.

    FOR pos IN pconMap:KEYS {
      LOCAL posData IS pconMap[pos].
      SET pNeurons[posData[0]] TO posData[1][gameBoardData[pos]].
    }
  }
}

FUNCTION neural_network_update {
  PARAMETER n0Neurons,move,conversionMap.
  FOR pType IN LIST("X","O") {
    LOCAL notpType IS otherPlayer[pType].
    LOCAL pNeurons IS n0Neurons[pType].
	//PRINT pNeurons.
...
FUNCTION final_node {
  PARAMETER prevousNeurons,functionConstants,outputIndex.
  //PRINT "oi " + outputIndex.
  LOCAL functionIndex IS outputIndex * (prevousNeurons:LENGTH + 1).
  //PRINT "fi " + functionIndex.
  LOCAL newNeuron IS functionConstants[functionIndex].
  SET functionIndex TO functionIndex + 1.
  FOR preNeuron IN prevousNeurons {
    SET newNeuron TO newNeuron + preNeuron * functionConstants[functionIndex].
    SET functionIndex TO functionIndex + 1.
  }
  RETURN segmoid(newNeuron).
}

// FUNCTION network_stage {
  // PARAMETER prevousNeurons,functionConstants,numberOfOutputNodes.
  // LOCAL newNodes IS LIST().
  // LOCAL prevousNeuronsLength IS prevousNeurons:LENGTH.
  // LOCAL functionIndex IS 0.
  // FROM { LOCAL i IS 0. } UNTIL i >= numberOfOutputNodes STEP { SET i TO i + 1. } DO {
    // LOCAL newNeuron IS functionConstants[functionIndex].
    // SET functionIndex TO functionIndex + 1.
    // FOR preNeuron IN prevousNeurons {
      // SET newNeuron TO newNeuron + preNeuron * functionConstants[functionIndex].
      // SET functionIndex TO functionIndex + 1.
    // }
    // newNodes:ADD(segmoid(newNeuron)).
  // }
  // RETURN newNodes.
// }
