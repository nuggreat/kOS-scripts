...
LOCAL pathETAstring IS time_formatting(etaListStatic[waypointIndex],5).
LOCAL oldSlope IS 0.
...
UNTIL done {
	...
	IF ((distToWay < waypointRadius) OR (pointMag = 1)) AND (NOT stopping) AND (waypointIndex < wayListLength) {
		...
		IF waypointIndex > wayListLength { SET stopping TO TRUE. SET waypointIndex TO waypointIndex - 1. } ELSE {
			...
			SET pathETAstring TO time_formatting(etaListStatic[waypointIndex],5).
...
FUNCTION screen_update {
	...
	pList:ADD("Distance : " + si_formatting(tarDist,"m")).
	//pList:ADD("ETA      : " + time_formatting(average_eta(tarDist - stopDist),5)).
