...
CLEARSCREEN.
IF NOT skipConfirm {
	PRINT "Launching to an apoapsis of: " + (targetAP / 1000) + "Km".
	IF bodyAtm:EXISTS AND targetAP < bodyAtm:HEIGHT {
		PRINT "Warning target apoapsis is below atmosphere height".
		PRINT " ".
	}
	PRINT "Launching with heading of:    " + launchHeading.
	PRINT " ".
	IF bodyAtm:EXISTS {
