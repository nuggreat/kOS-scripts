...
FUNCTION node_step_init {
	...
	FOR nStep IN stepOrder {
		IF nStep = "eta" {
			stepLex:ADD(numKey,{ PARAMETER targetNode,stepMag. SET targetNode:ETA TO targetNode:ETA + stepMag. }).
			stepLex:ADD(-numKey,{ PARAMETER targetNode,stepMag. SET targetNode:ETA TO targetNode:ETA - stepMag. }).
		} ELSE IF nStep = "pro" {
			stepLex:ADD(numKey,{ PARAMETER targetNode,stepMag. SET targetNode:PROGRADE TO targetNode:PROGRADE + stepMag. }).
			stepLex:ADD(-numKey,{ PARAMETER targetNode,stepMag. SET targetNode:PROGRADE TO targetNode:PROGRADE - stepMag. }).
		} ELSE IF nStep = "norm" {
			stepLex:ADD(numKey,{ PARAMETER targetNode,stepMag. SET targetNode:NORMAL TO targetNode:NORMAL + stepMag. }).
			stepLex:ADD(-numKey,{ PARAMETER targetNode,stepMag. SET targetNode:NORMAL TO targetNode:NORMAL - stepMag. }).
		} ELSE IF nStep = "rad" {
			stepLex:ADD(numKey,{ PARAMETER targetNode,stepMag. SET targetNode:RADIALOUT TO targetNode:RADIALOUT + stepMag. }).
			stepLex:ADD(-numKey,{ PARAMETER targetNode,stepMag. SET targetNode:RADIALOUT TO targetNode:RADIALOUT - stepMag. }).
		}
...
FUNCTION node_step_full { //manipulates the targetNode in one of 4 ways depending on manipType for a value of stepVal
	PARAMETER targetNode,stepDir,stepMag.
	hill_climb_lex["stepOrder"][stepDir](targetNode,stepMag).
}
