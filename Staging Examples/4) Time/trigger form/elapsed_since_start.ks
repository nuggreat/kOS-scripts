...
// The trigger will stage based on when enough time has passed since the the function was called the timeing is set by the first parameter
// The function takes 2 parameters the first is manditory the second is optional
...
//  The second parameter is a boolean defaulted to TRUE
...
//  The key "stagesLeft" when called will return the number of staging events left in the programed sequence
...
//   The returned value will be positave when the next staging event is in the futuer and negitave if staging time was missed
