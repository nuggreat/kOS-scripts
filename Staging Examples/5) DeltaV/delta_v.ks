...
//The staging_check function compares the current deltaV in the stage and current time against a configuration and stages when all criteral are met
// The first condition that must be met is that the deltaV in the current stage must be below the spesffied threshold
// The second condition that must be met is that the deltaV must stay below the threshold for a given number of seconds
// The third condition that must be met is that enough time must have passed between the current staging attempt and the past secussful staging
// The function takes one parameter which is the configuation generated by the staging_start function
