...
LOCAL FUNCTION time_string {
  ...
  // add padding_v2 to each element in timeList and prepend to returnString
  ...
  UNTIL i >= maxLength STEP {SET i TO i + 1.} DO {
    ...
    LOCAL pad_str IS padding_v2(timeList[i], leading0List, roundingList[i], FALSE, 1).
    SET returnString TO pad_str + stringList[i] + returnString.
  }

  // the prependT strings have one space padding_v2
  IF prependT SET returnString TO returnString:INSERT(0, " ").

  // all time_string formats have either '+','-', or ' ' for padding_v2
  IF timeSec < 0 {
...
FUNCTION si_formatting_v2 {
  ...
  IF num = 0 {
    ...
    RETURN padding_v2(num/1000^SIfactor,1,trailingLength,TRUE,0) + prefix + unit.
  }
}

lib_formatting_lex:ADD("roundingFunctions",LIST(ROUND @,FLOOR @,CEILING @)).
FUNCTION padding_v2  {
  ...
  // } ELSE IF roundType = 1 {
    // SET returnString TO ABS(FLOOR(num,trailingLength)):TOSTRING.
  // } ELSE {
    // SET returnString TO ABS(CEILING(num,trailingLength)):TOSTRING.
  // }
  LOCAL returnString IS ABS(lib_formatting_lex["roundingFunctions"][roundType](num,trailingLength)):TOSTRING.

  IF trailingLength > 0 {
    IF returnString:CONTAINS(".") {
	  LOCAL splitString IS returnString:SPLIT(".").
	  //IF splitString[0]:LENGTH < leadingLength OR splitString[1]:LENGTH < trailingLength {
	  //IF splitString[1]:LENGTH < trailingLength OR splitString[0]:LENGTH < leadingLength {//17
	    SET returnString TO (splitString[0]:PADLEFT(leadingLength) + "." + splitString[1]:PADRIGHT(trailingLength)):REPLACE(" ","0").
	  //}
	  // IF splitString[0]:LENGTH < leadingLength {
        // SET returnString TO (splitString[0]:PADLEFT(leadingLength)+ "."):REPLACE(" ","0").
	  // } ELSE {
	    // SET returnString TO splitString[0] + ".".
	  // }
      // IF splitString[1]:LENGTH < trailingLength {
	    // SET returnString TO returnString + (splitString[1]:PADRIGHT(trailingLength)):REPLACE(" ","0").
	  // } ELSE {
	    // SET returnString TO returnString + splitString[1].
	  // }
